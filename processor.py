...
def default_inputs():
    ...
    #
    numeric_terms = ['_meters', '_count', 'minimum_', 'maximum_', '_minimum_', '_maximum', 'decimal', 'in_meters']
                ...
                for item_file in item['files']:
                    ...
                    if pass_flag:
                        ...
                        if commands['fetch_csvs']:
                            if not commands['file_overwrite'] and os.path.isfile(
                                os.path.join(commands['source_data_dir'], src_file)):
                                logger.debug("    {}".format('File was previously downloaded'))
...
def regenerate_csv(fpaths, commands, err_msg = [], p_tasks = []):
    ...
    if not os.path.isfile(fpaths['rconpath']) or commands['proc_overwrite']: 
        ...
        if ndiv == 1:
            ...
            logger.debug("\t{}".format('Source data file reconstructed from NetCDF in one read'))
            ...
            logger.debug("\t{}{}".format('Source data file reconstructed from NetCDF in chunks: ', ndiv))
        ...
        if os.path.isfile(fpaths['sfpath']) and os.path.isfile(fpaths['rconpath']):
                ...
                if match_f1:
                    ...
                    if commands['error_report']:
                        ...
                        with open(fpaths['cmp_report'], "w") as ewrite:
                            disclaimer = "{}\n\t{}\n\t{}\n\t{}\n\t".format(
                                '*** Purpose: This is a preliminary processing file with simple and intuitive results.',
                                'A source csv file was evaluated and converted into NetCDF and then converted back to a *.csv file.',
                                'Direct comparisons were made between the source csv and regenerated csv files.',
                                'This information will be organized differently in the future.')
            ...
            if dump_csv:
                if os.path.exists(fpaths['rconpath']): 
                    logger.debug("\t{}{}\n".format('deleting reconstructed csv file, dump_csv = ', dump_csv))
                    os.remove(fpaths['rconpath'])   
        else:
...
def meta_proc(filename, metadata = {}):
    ...
    attrs['history'] =         fmt.format(ItemStartDate,
           ...
           FileStartDate,                               
           'Phase II - Initial QA/QC reprocessing to Darwin Core Standards.',
           DateNowUtc, 
                ...
                if err[0] == err[1]:
                    if err[1][2] > err[0][2]:
                        if commands['netcdf_type'] != 'NETCDF3_CLASSIC':
                            ...
                            plogger.debug("\t\t\t{}".                                   format('action: prescribe larger byte size'))     
                        else:
...
def csv_to_nc_dframe(filename, metadata, fpaths, commands, 
                     dc_vocab = [],  p_tasks = [], err_msg = []):
    
    logger.debug("\t{}".format('Implementing dataframe (Xarray, Dask) file conversion method'))
    ...
    if not os.path.isfile(fpaths['ncpath']) or commands['proc_overwrite']: 
        ...
        with open(fpaths['sfpath'], encoding = commands['string_fmt'],
            ...
            timevar = [variable for variable in commands['LLAT_specs']                     for key, value in commands['LLAT_specs'][variable].items() if value == 'time']
            posvar = [variable for variable in commands['LLAT_specs']                     for key, value in commands['LLAT_specs'][variable].items() if value != 'time']
            quantvar = [] 
            stringvar = [] 
            for label in df_labels:
            ...
            for df in dframe:
               ...
               # -------------------------------------------------
                ...
                if columns == bulk_stat['columns']:
                    for i, col in enumerate(df.columns):
                        ...
                        # variable should be converted to string 
                                ...
                                msg = '\n{}{}{}'.format('WARNING: - variable: ', df_labels[i],                                                         ' read as numeric, forcing conversion to string' )
                                err_msg.append(msg)
                                try:
                ...
                if not commands['chunksize'] or bulk_stat['rows'] <= commands['chunksize']:
                    dataframe_metadata(ds, metadata, dc_vocab, commands, err_msg, df_labels,  
            ...
            if commands['chunksize']: 
                if bulk_stat['rows'] > commands['chunksize']:
                    ...
                    p_tasks.append('processed data in multiple chunks (parts)')
                    commands['QC_pass'] = True
                    decodeCF = True
                    ...
                    if commands['cf_comply']:
                        try:
                            ...
                            ds = ds.chunk(chunks=commands['chunksize'], name_prefix='xarray-', 
                                          token=None, lock=True)
                            dataframe_metadata(ds, metadata, dc_vocab, commands, err_msg, 
...
def setup_fpaths(filename, commands, p_tasks = [], err_msg = []):
    ...
    if os.path.exists(fpaths['sfpath']):
        p_tasks.append('Source file locally identified')
    ...
    os.remove(fpathss['pre_sfpath']) if os.path.exists(fpaths['pre_sfpath']) else None
    return fpaths
...
def run(**kwargs):
    ...
    # -------------------------------------------------------
    if commands['darwin_vocab']:
        dc = open(commands['darwin_vocab'], 'r', 
                  encoding= commands['string_fmt'], errors='replace')
        dc_vocab = json.load(dc)
    else:
        dc_vocab = []
    ...
    # 4) optionally create the ERDDAP datasets.xml file using netCDF files
    # ---------------------------------------------------------------------
